...
> = {
  ...
  luxe: {
    ...
    color: "D4AF37",
    icon: "https://ui-avatars.com/api/?name=Luxe&background=D4AF37&color=fff",
  },
  bhadipa: {
    title: "Bhadipa & Bha2pa",
    color: "FF5722",
    icon: "https://ui-avatars.com/api/?name=Bhadipa&background=FF5722&color=fff",
  },
  vishaykhol: {
    title: "Vishaykhol",
    color: "9C27B0",
    icon: "https://ui-avatars.com/api/?name=Vishay&background=9C27B0&color=fff",
  },
  puppycuddles: {
    title: "PuppyCuddles",
    color: "FF9800",
    icon: "https://ui-avatars.com/api/?name=Puppy&background=FF9800&color=fff",
  },
};
          ...
          {reels.map((reel, index) => {
            ...
            return (
              <Reveal
                ...
                direction="up"
                width="fit-content"
                className="snap-center shrink-0 first:ml-0 md:first:ml-4 last:mr-0 md:last:mr-4"
              >
                <div className="w-[240px] sm:w-[250px] flex flex-col bg-zinc-900 rounded-md overflow-hidden relative shadow-lg group">
                  ...
                  <a
                    ...
                    className="relative h-[360px] sm:h-[380px] w-full overflow-hidden block"
                  >
                    <img
                      src={reel.thumb}
                    ...
                    <div className="absolute bottom-3 left-3 right-3 flex items-center gap-2.5 pointer-events-none">
                      <img
                        src={config.icon}
                        alt={`${config.title} Brand Icon`}
                        loading="lazy"
                        decoding="async"
                        className="w-8 h-8 rounded-sm border border-white/50 object-cover shadow-sm"
                      />
                      <div className="flex flex-col">
                        <span className="text-white font-bold text-[13px] leading-tight drop-shadow-md">
                          {config.title}
                        </span>
                        <span className="text-white/90 text-[11px] font-medium drop-shadow-md">
                          Reel {index + 1}
                        </span>
                      </div>
                    </div>
                  ...
                  {/* Action Button Area */}
                  <div
                    className="flex h-[42px] text-white"
...
export const RecentWorkSection: React.FC = () => {
  ...
  return (
    <section
      id="recent-work"
      className="py-20 md:py-28 px-0 md:px-12 lg:px-24 border-t border-zinc-500/10 relative overflow-hidden"
    >
        ...
        <Reveal>
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-10 md:mb-16 gap-6 px-6 md:px-0">
            <div>
              ...
              <h2 className="text-4xl md:text-5xl font-serif mb-4 text-fluid-h2 font-bold">
                Recent Work
              </h2>
              <p className="text-zinc-500 max-w-xl text-sm md:text-base font-light leading-relaxed">
                A collection of high-retention, viral-engineered reels produced
                ...
                <div className="absolute bottom-0 left-0 right-0 p-5 md:p-10 flex flex-col md:flex-row md:items-end justify-between gap-4 md:gap-6">
                  <div className="max-w-2xl px-2 md:px-0">
                    <div className="flex items-center gap-2 md:gap-3 mb-2 md:mb-4">
                      <img
                        src={BRAND_CONFIG.bni.icon}
                        alt="BNI Inesh Brand Icon"
                        loading="lazy"
                        decoding="async"
                        className="w-8 h-8 md:w-10 md:h-10 rounded-sm border border-white/20"
                      />
                      <span className="text-[#0D8ABC] font-black uppercase tracking-[0.2em] md:tracking-[0.3em] text-[10px] md:xs">
                        BNI Inesh
                      </span>
                    </div>
                    <h4 className="text-xl sm:text-2xl md:text-4xl font-serif font-bold text-white mb-2 md:mb-3 leading-tight">
                      The Vision of BNI Inesh
                    </h4>
                    <p className="text-zinc-300 text-[11px] sm:text-xs md:text-base font-light max-w-lg line-clamp-3 md:line-clamp-none">
                      A cinematic showcase for the biggest networking group in
                      Navi Mumbai. Captured the essence of collaboration,
