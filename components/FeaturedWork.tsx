...
export const FeaturedWork: React.FC = () => {
  const combinedWorks = [
    ...PROJECTS.map(p => ({ title: p.title, category: p.deliverable, image: p.image })),
    ...BRAND_PROJECTS.map(p => ({ title: p.name, category: p.category, image: p.image }))
  ].slice(0, 4);
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 lg:gap-24">
          {combinedWorks.map((work, i) => (
            <motion.div
              ...
              transition={{ duration: 0.8, ease: [0.76, 0, 0.24, 1], delay: i % 2 === 0 ? 0 : 0.2 }}
            ...
            >
              ...
              <div className="flex justify-between items-center px-2">
                <h3 className="text-xl md:text-2xl font-bold tracking-tight text-white">{work.title}</h3>
                <span className="text-[10px] sm:text-xs tracking-widest uppercase text-zinc-500 border border-white/10 px-2 sm:px-3 py-1 rounded-full text-center">{work.category}</span>
              </div>
            </motion.div>
          ))}
        </div>
