      ...
      <div className="relative bg-[var(--bg-color)] text-[var(--text-color)] w-full h-full md:h-auto md:max-h-[90vh] md:max-w-6xl overflow-y-auto md:rounded-3xl shadow-[0_0_100px_rgba(0,0,0,0.5)] flex flex-col md:flex-row md:border md:border-zinc-500/10 z-10 transition-colors duration-800 scrollbar-hide">
        ...
        <div className="w-full md:w-1/2 p-6 sm:p-10 md:p-12 lg:p-16 flex flex-col">
          <div className="mb-8">
            ...
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-serif font-bold leading-tight mb-4 sm:mb-6">
              {project.title}
            </h2>
          ...
          <div className="space-y-10 flex-grow">
            {project.challenges && (
              <div>
                <h3 className="text-[10px] uppercase font-black tracking-[0.4em] text-zinc-400 mb-4">
                  The Challenge
                </h3>
                <ul className="space-y-3">
                  {project.challenges.map((challenge, i) => (
                    <li
                      key={i}
                      className="text-zinc-500 text-xs sm:text-sm flex gap-4 leading-relaxed"
                    >
            ...
            <div className="grid grid-cols-2 gap-6 pt-8 border-t border-zinc-500/10 mb-8 md:mb-0">
              <div>
                <span className="text-[8px] uppercase font-black tracking-widest text-zinc-400 block mb-1.5">
                  Deliverable
                </span>
                <p className="text-zinc-500 text-[10px] sm:text-xs font-medium uppercase tracking-tighter leading-tight">
                  {project.deliverable}
                </p>
              </div>
              <div>
                ...
                <p className="text-current text-[10px] sm:text-xs font-bold leading-tight">
                  {project.impact}
                </p>
              </div>
            </div>
