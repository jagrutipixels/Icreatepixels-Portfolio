...
const services = [
  {
    ...
    description: "Cinematic quality storytelling bridging physical sets and digital screens.",
  ...
  {
    title: "Web & Digital",
    description: "Lightning-fast, highly-converting digital infrastructure and UI.",
      ...
      <div className="max-w-[1400px] mx-auto grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-24 relative">
        ...
        <div className="lg:col-span-4 lg:sticky lg:top-32 h-max">
          ...
          <p className="text-zinc-500 max-w-sm">
            We don't just produce videos. We architect complete digital experiences engineered for maximum conversion, audience retention, and aesthetic dominance.
          </p>
        </div>
        ...
        <div className="lg:col-span-8 grid grid-cols-1 md:grid-cols-3 gap-4">
          {services.map((service, i) => (
            ...
            >
              <div>
                <h3 className="text-2xl font-bold tracking-tight text-white mb-3">{service.title}</h3>
                <p className="text-zinc-400 text-sm">{service.description}</p>
              </div>
              
              <div className="flex flex-wrap gap-2 mt-8">
                {service.tags.map((tag, j) => (
                  <span key={j} className="text-[10px] uppercase tracking-widest px-3 py-1 bg-white/5 border border-white/10 rounded-full text-zinc-300">
                    {tag}
                  </span>
                ))}
              </div>
