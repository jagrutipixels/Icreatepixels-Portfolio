  ...
  return (
    <div className="relative">
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden -z-10 opacity-30">
        ...
        <div
          className="absolute bottom-[20%] right-[10%] w-[40%] h-[40%] bg-purple-500/10 blur-[150px] rounded-full animate-drift"
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-stretch">
              ...
              {SKILLS[0].items.map((item, i) => (
                <div key={i} className="group">
                  <div className="flex justify-between items-end mb-3">
                    <span className="text-sm text-zinc-400 group-hover:text-zinc-100 transition-colors duration-500">
                      {item}
                    </span>
        ...
        <Reveal className="md:col-span-4" direction="up" delay={100}>
          <div className="h-full p-6 sm:p-10 bg-zinc-900/40 backdrop-blur-md rounded-[2rem] sm:rounded-[2.5rem] border border-zinc-800/50 flex flex-col text-left">
            <h3 className="text-[10px] font-black uppercase tracking-[0.4em] text-zinc-100 mb-8 sm:mb-12">
              Visual Identity
            </h3>
            <ul className="space-y-4 sm:space-y-6">
              {SKILLS[1].items.map((item, i) => (
                <li key={i} className="flex items-center gap-4 group">
                  ...
                  <span className="text-sm text-zinc-500 group-hover:text-zinc-300 transition-colors duration-500">
                    {item}
                  </span>
        ...
        <div className="md:col-span-3 flex flex-col gap-6">
          ...
          <Reveal className="flex-grow" direction="up" delay={200}>
            <div className="h-full p-6 sm:p-8 bg-zinc-900/40 backdrop-blur-md rounded-[2rem] sm:rounded-[2.5rem] border border-zinc-800/50 text-left">
              <h3 className="text-[10px] font-black uppercase tracking-[0.4em] text-zinc-500 mb-6">
                Post-Process
              </h3>
          ...
          <Reveal direction="up" delay={300}>
            <div className="p-6 sm:p-8 bg-white text-black rounded-[2rem] sm:rounded-[2.5rem] shadow-2xl transition-transform duration-500 hover:scale-[1.02] text-left">
              <h3 className="text-[10px] font-black uppercase tracking-[0.4em] opacity-40 mb-6">
                Strategy
              </h3>
              <ul className="space-y-2.5 sm:space-y-3">
                {SKILLS[3].items.map((item, i) => (
                  <li
                    key={i}
                    className="text-[11px] font-black uppercase tracking-tight leading-none hover:translate-x-1 transition-transform cursor-default"
                  >
                    {item}
                  </li>
                ))}
      ...
      <div className="mt-24 pt-12 border-t border-zinc-800/30">
        <Reveal>
          <div className="flex items-center gap-6 mb-16">
            <h3 className="text-[10px] font-black uppercase tracking-[0.6em] text-zinc-500 whitespace-nowrap">
